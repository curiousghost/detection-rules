...
from detection_engineering.chronicle.common import chronicle_auth
from detection_engineering.chronicle.detection_engine.v2.verify_rule import verify_rule
from detection_engineering.common import RuleVerificationError
...
def verify_rule_text(rule_file: pathlib.Path):
  ...
  response = verify_rule(http_session=http_session, rule_content=rule_text)

  if response.get("success"):
    LOGGER.info(
        "Rule verified successfully (%s). Context: %s",
        rule_file,
        response["context"],
    )
    ...
    raise RuleVerificationError(
        f"Rule verification error ({rule_file}). Context: {response['context']}"
    )
...
def verify_rules():
  ...
  for rule_file in list(RULES_DIR.glob("*.yaral")):
    ...
    if response.get("success") is True:
      ...
      raise RuleVerificationError(
          f"Rule verification error ({rule_file}). Context:"
          f" {response['context']}"
      )


if __name__ == "__main__":
  LOGGER.info("detection engineering cli started")

  parser = argparse.ArgumentParser(description="detection_engineering cli")
